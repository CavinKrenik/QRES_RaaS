# Documentation Index

Quick navigation for QRES v20.0.1 documentation.

## Core Documentation

| Category | Contents |
|----------|----------|
| **[reference/](reference/)** | ARCHITECTURE, API_REFERENCE, SPEC, SCOPE |
| **[status/](status/)** | IMPLEMENTATION_STATUS, TECHNICAL_DEBT |
| **[verification/](verification/)** | QRES_V20_FINAL_VERIFICATION, MULTIMODAL_* reports |
| **[security/](security/)** | INVARIANTS, SECURITY_ROADMAP, TEE_MIGRATION_GUIDE, COGNITIVE_MESH_SAFETY |
| **[benchmarks/](benchmarks/)** | BENCHMARKS, CLOUD_BENCHMARK_RESULTS |
| **[roadmap/](roadmap/)** | ROADMAPv20, COGNITIVE_MESH_ROADMAP, RaaS (future) |

## Guides & Research

| Category | Contents |
|----------|----------|
| **[guides/](guides/)** | QUICK_START, API_COOKBOOK, CONTRIBUTING, IMAGES, P2P_IMPLEMENTATION |
| **[research/](research/)** | RELATED_WORK, SENSITIVITY_ANALYSIS, COST_COMPARISON |
| **[theory/](theory/)** | Formal specifications and proofs |
| **[adrs/](adrs/)** | Architecture Decision Records |

## Supporting Materials

| Category | Contents |
|----------|----------|
| **[power/](power/)** | Energy budget analysis |
| **[deployment/](deployment/)** | Deployment configurations |
| **[systems/](systems/)** | System architecture diagrams |
| **[images/](images/)** | Documentation images |
| **[archive/](archive/)** | Historical/superseded documentation |

## Quick Links

- [README](../README.md) - Project overview
- [CHANGELOG](../CHANGELOG.md) - Version history
- [Implementation Status](status/IMPLEMENTATION_STATUS.md) - Feature tracking
- [Security Invariants](security/INVARIANTS.md) - INV-1 through INV-7

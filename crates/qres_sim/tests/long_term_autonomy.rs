use qres_core::{
    adaptive::regime_detector::{RegimeDetector, Regime as RegimeState},
    power::MockRadio,
...
fn test_6month_autonomy() {
    ...
    if !std::path::Path::new(dataset_path).exists() {
        panic!("Dataset not found at {}. Please run fetch_weather_6month.py first.", dataset_path);
    }

    let samples = load_weather_samples(dataset_path)
        .expect("Failed to load 6-month dataset");
    
    println!("Loaded {} samples ({:.1} days)", 
             samples.len(), 
    ...
    for (idx, sample) in samples.iter().enumerate() {
        ...
        let transmissions_this_step = if intervals_per_sample >= 1.0 {
            intervals_per_sample as u64
             ...
             if stride > 0 && idx % stride == 0 { 1 } else { 0 }
        };
        ...
        if idx % (144 * 30) == 0 && idx > 0 {
            ...
            println!("  Day {}: Regime={:?}, Energy={}J, Transmissions={}, Runtime={:.1}s",
                     days, effective_regime, energy_pool.current(), 
                     transmission_count, elapsed.as_secs_f64());
        }
    }
    ...
    println!("Final energy: {}J / 23760J", energy_history.last().unwrap().1);
    println!("Avg reputation: {:.3}", final_avg_reputation);
    
    // Regime distribution
    let calm_count = regime_history.iter().filter(|(_, r)| matches!(r, RegimeState::Calm)).count();
    let prestorm_count = regime_history.iter().filter(|(_, r)| matches!(r, RegimeState::PreStorm)).count();
    let storm_count = regime_history.iter().filter(|(_, r)| matches!(r, RegimeState::Storm)).count();
    
    println!("\nRegime distribution:");
    println!("  Calm: {:.1}%", 100.0 * calm_count as f64 / regime_history.len() as f64);
    println!("  PreStorm: {:.1}%", 100.0 * prestorm_count as f64 / regime_history.len() as f64);
    println!("  Storm: {:.1}%", 100.0 * storm_count as f64 / regime_history.len() as f64);
    ...
    // Assertions for CI
    assert!(energy_history.last().unwrap().1 > 1000.0, "Battery depleted before 6 months");
    assert!(transmission_count > 100, "Too few transmissions");
    assert!(transmission_count < samples.len() as u64 * 10, "Transmission sanity check");
}

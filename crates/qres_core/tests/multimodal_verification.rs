...
use qres_core::multimodal::{MultimodalFusion, Modality, ATTENTION_WINDOW};
...
fn test_deterministic_bit_check() {
    ...
    assert_eq!(pred1.mantissas.len(), pred2.mantissas.len(), "Dimensions must match");
...
fn test_energy_gate_check() {
    ...
    let pred_critical = fusion.predict_with_attention(Modality::Temperature, reputation_in_critical);
    ...
    let differs = pred_critical.mantissas.iter()
        .zip(pred_normal.mantissas.iter())
        .any(|(a, b)| a != b);
    
    assert!(differs, "Critical energy mode should alter predictions via reputation weighting");
    
    println!("✓ Energy gate check PASSED (critical={}, low={})", 
        energy.is_critical(), energy.is_low());
}
...
fn test_cross_modal_surprise() {
    ...
    let differs = pred_biased.mantissas.iter()
        .zip(pred_baseline.mantissas.iter())
        .any(|(a, b)| (a - b).abs() > 100); // Significant difference
    
    assert!(differs, "High temperature surprise should bias humidity prediction");
...
fn test_counter_based_lr_scaling() {
    ...
    assert!(new_lr < initial_lr, "LR scale should decrease after persistent imbalance");
    assert!(new_lr > 0.5, "LR scale should not collapse completely");
    
    println!("✓ Counter-based LR scaling PASSED (initial={:.3}, new={:.3})", initial_lr, new_lr);
}
...
fn test_lr_scaling_high_variance() {
    ...
    assert!(final_lr >= 0.59, "LR should respect min threshold (~0.6) even under sustained high error (got {:.3})", final_lr);
    assert!(final_lr <= 0.61, "LR should reach the floor, not exceed it significantly (got {:.3})", final_lr);
    
    println!("✓ High-variance stress PASSED (final LR={:.3}, min=0.6 floor enforced)", final_lr);
}
...
fn test_reputation_weighting() {
    ...
    assert!(high_f32[0] > low_f32[0], "High reputation should produce larger predictions");
    ...
    assert!(influence_ratio < 0.5, "Low reputation influence should be significantly attenuated");
...
fn test_memory_overhead() {
    ...
    let fusion = MultimodalFusion::new(4); // Max modalities
    ...
    println!("MultimodalFusion memory estimate: ~{} bytes", total_estimate);
    
    // Should be well under 1MB (embedded constraint)
    assert!(total_estimate < 1_000_000, "Memory usage too high for embedded");
...
fn test_event_driven_attention_heap() {
    ...
    assert!(heap_bytes < 2048, "Heap footprint should be < 2KB (got {} bytes)", heap_bytes);
    ...
    assert!(!pred.mantissas.is_empty(), "Prediction should work after spike");
    ...
    assert!(max_pred_magnitude < 1000.0,
        "Predictions should be bounded (got max={:.2})", max_pred_magnitude);

    // Final heap check: should still be compact
    let final_heap = fusion.estimated_heap_bytes();
    assert!(final_heap < 2048, "Final heap should remain < 2KB (got {} bytes)", final_heap);

    println!("Event-driven attention heap PASSED");
    println!("  Heap: {} bytes (initial), {} bytes (final)", heap_bytes, final_heap);
...
fn test_influence_cap_under_slander() {
    ...
    assert!((pre_slander_score - 0.9).abs() < 0.01, "Pre-slander R should be ~0.9");
    ...
    assert!(post_slander_influence > 0.3, "Post-slander influence should remain substantial");

    // Verify influence drop is bounded (less than 60% reduction)
    let influence_ratio = post_slander_influence / pre_slander_influence;
    assert!(influence_ratio > 0.4, "Influence ratio should be > 0.4 (bounded slander impact)");
    ...
    assert!((max_influence - 0.8).abs() < 0.001, "R=1.0 influence must be capped at 0.8");
    ...
    assert!((fixed_as_f32 - 0.8).abs() < 0.001, "Fixed-point influence must match f32");

    println!("Influence cap under slander PASSED");
    println!("  Pre-slander: R={:.3}, influence={:.4}", pre_slander_score, pre_slander_influence);
    println!("  Post-slander: R={:.3}, influence={:.4}", post_slander_score, post_slander_influence);

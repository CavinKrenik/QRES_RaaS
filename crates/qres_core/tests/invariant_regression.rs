...
fn inv1_bounded_influence() {
    ...
    let aggregator = WeightedTrimmedMeanAggregator::new(2, reputations);
    ...
    println!("INV-1: Adversary R=0.01, bias=100 → drift={:.4}", drift);
    assert!(
        drift < 0.15,
...
fn inv2_sybil_resistance() {
    ...
    let aggregator_baseline = WeightedTrimmedMeanAggregator::new(10, reputations_baseline);
    let result_baseline = aggregator_baseline.aggregate(&updates_baseline);
    let drift_baseline = (result_baseline.weights[0] - 1.0).abs();
    ...
    let aggregator_sybil = WeightedTrimmedMeanAggregator::new(10, reputations_sybil);
    let result_sybil = aggregator_sybil.aggregate(&updates_sybil);
    let drift_sybil = (result_sybil.weights[0] - 1.0).abs();

    // Verify: drift increase is sub-linear relative to attacker count doubling.
    // With fixed trim count f=10, more Sybils survive trimming, so some
    // amplification above 2x is expected. The economic bound ensures it stays
    // well below the naive 2x linear scaling when reputation dilution is factored in.
    let drift_ratio = if drift_baseline > 0.0 {
        drift_sybil / drift_baseline
    } else {
        0.0
    };

    println!(
        "INV-2: Baseline drift={:.4}, Sybil drift={:.4}, ratio={:.2}x",
        drift_baseline, drift_sybil, drift_ratio
    );

    assert!(
        drift_ratio < 2.5,
        "INV-2 VIOLATED: Sybil attack amplified drift by {:.2}x (expected < 2.5x)",
        drift_ratio
    );
}
...
fn inv3_collusion_graceful() {
    ...
    let aggregator = WeightedTrimmedMeanAggregator::new(8, reputations.clone());
    let result_independent = aggregator.aggregate(&updates_independent);
    let drift_independent = (result_independent.weights[0] - 1.0).abs();
    ...
    let result_colluding = aggregator.aggregate(&updates_colluding);
    let drift_colluding = (result_colluding.weights[0] - 1.0).abs();

    // Verify: collusion amplifies but is bounded (< 5x)
    let amplification = if drift_independent > 0.0 {
        drift_colluding / drift_independent
    } else {
        0.0
    };

    println!(
        "INV-3: Independent drift={:.4}, Colluding drift={:.4}, amplification={:.2}x",
        drift_independent, drift_colluding, amplification
    );
...
fn inv4_regime_gate() {
    ...
    let low_rep_signals = [0.3f32; 10]; // All below threshold
    let high_rep_count = low_rep_signals
        .iter()
        .filter(|&&r| r > REPUTATION_THRESHOLD)
        .count();

    println!(
        "INV-4: Low-rep signals: {} nodes with R > {}",
        high_rep_count, REPUTATION_THRESHOLD
    );
    assert!(
    ...
    let high_rep_count_mixed = mixed_signals
        .iter()
        .filter(|&&r| r > REPUTATION_THRESHOLD)
        .count();

    println!(
        "INV-4: Mixed signals: {} high-rep nodes (need {})",
        high_rep_count_mixed, QUORUM_MIN
    );
    ...
    let high_rep_signals = [0.9f32; 3];
    let high_rep_count_ok = high_rep_signals
        .iter()
        .filter(|&&r| r > REPUTATION_THRESHOLD)
        .count();

    println!(
        "INV-4: High-rep quorum: {} nodes (threshold met)",
        high_rep_count_ok
    );
...
///
/// Formal: BFP16 fixed-point produces identical results across platforms
/// Test: Same input → same output (no floating-point drift)
#[test]
fn inv6_determinism() {
    // Test vectors for BFP16 round-trip arithmetic
    let test_values = vec![1.0f32, 0.5, 0.25, 1e-5, 100.0];

    for val in &test_values {
        // Round-trip: float → BFP16 → float
        let bfp = Bfp16Vec::from_f32_slice(&[*val]);
        let recovered = bfp.to_vec_f32()[0];

        // BFP16 precision depends on the shared exponent; allow reasonable error
        let error = (recovered - val).abs();
        let relative_error = if val.abs() > 0.0 {
            error / val.abs()
        } else {
            error
        };
        ...
        println!(
            "INV-6: {:.6} → BFP16 → {:.6} (error: {:.8}, relative: {:.8})",
            val, recovered, error, relative_error
        );

        assert!(
            relative_error < 0.01, // 1% relative error tolerance for BFP16
            "INV-6 VIOLATED: BFP16 round-trip relative error {:.8} exceeds bound for value {}",
            relative_error,
            val
        );
    }
    ...
    for i in 0..bfp_vec.mantissas.len() {
        assert_eq!(
            bfp_vec.mantissas[i], bfp_vec2.mantissas[i],
            "INV-6 VIOLATED: Non-deterministic conversion at index {}",
            i
        );
    }
    assert_eq!(
        bfp_vec.exponent, bfp_vec2.exponent,
        "INV-6 VIOLATED: Non-deterministic exponent conversion"
    );
}

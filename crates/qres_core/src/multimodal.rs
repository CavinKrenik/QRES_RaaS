...
impl MultimodalFusion {
    ...
    pub fn new(num_modalities: usize) -> Self {
        assert!(num_modalities <= MAX_MODALITIES, "Max 4 modalities supported");
        
        // Initialize empty history (ring buffer)
        let history = (0..num_modalities)
            .map(|_| {
                (0..ATTENTION_WINDOW)
                    .map(|_| Bfp16Vec { exponent: 0, mantissas: alloc::vec![] })
                    .collect()
            })
            .collect();
    ...
    pub fn observe(&mut self, modality: Modality, observation: Bfp16Vec, prediction_error: f32) {
        ...
        if count <= 1 {
            ...
            let delta = if new_surprise > old_mean {
                new_surprise - old_mean
            } else {
                old_mean - new_surprise
            };
            ...
            let threshold = new_mean.saturating_add(
                sigma.saturating_mul(SPIKE_THRESHOLD_MULTIPLIER) / 2,
            );
    ...
    pub fn predict_with_attention(&self, target_modality: Modality, reputation_weight: f32) -> Bfp16Vec {
        ...
        let dim = target_history.iter().rev()
        ...
        for d in 0..dim {
            prediction_f32[d] += bias_magnitude;
        }
    ...
    fn update_lr_scale(&mut self, modality_idx: usize) {
        ...
        for other_idx in 0..self.num_modalities {
            ...
            if self.imbalance_counters[modality_idx][other_idx] > 5 {
                ...
                self.lr_scales[modality_idx] = (self.lr_scales[modality_idx].wrapping_mul(scale_factor)) >> 16;
    ...
    pub fn estimated_heap_bytes(&self) -> usize {
        ...
        total += self.num_modalities * 1; // spike_active: bool
        total
    }
